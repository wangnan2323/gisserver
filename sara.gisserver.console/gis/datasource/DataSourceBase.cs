...
namespace sara.gisserver.console.gis.datasource
{
...
";
        ...
        {            
            ...
            using (Stream streamConfxml = new MemoryStream(Utility.GetEntryBytesFromZIPFile(path, "v101/Layers/conf.xml")))
            {
                ...
                {
                    throw new Exception(" 没有在文件【" + System.IO.Path.GetFileName(path) + "】的压缩路径【v101/Layers/conf.xml】下找到【conf.xml】文件");
                }
                //read conf.xml
            ...
            //用zip方式解压缩tpk文件，并读取cdi--也是一个xml--用来处理四至问题
            using (Stream streamConfcdi = new MemoryStream(Utility.GetEntryBytesFromZIPFile(path, "v101/Layers/conf.cdi")))
            {
                ...
                {
                    throw new Exception(" 没有在文件【" + System.IO.Path.GetFileName(path) + "】的压缩路径【v101/Layers/conf.cdi】下找到【conf.cdi】文件");
                ...
                {
                    throw new Exception("conf.cdi文件读取失败!" + e.Message + e.StackTrace);
                }
                tilingScheme.InitialExtent = tilingScheme.FullExtent;
            }
            #endregion
        ...
        {            
            ...
            using (HttpWebResponse response = (HttpWebResponse)request.GetResponse())
            {
                ...
                return sara.gisserver.console.gis.util.Utility.StreamToBytes(response.GetResponseStream());
            }
        }
...
FROM map JOIN images ON images.tile_id = map.tile_id";
        ...
        protected void InternalOnTileLoaded(object o, TileLoadEventArgs a)
        {
            if (a.GeneratedMethod != TileGeneratedSource.DynamicOutput)
                return;
            if (o is DataSourceRasterImage && !sara.gisserver.console.config.ConfigManager.App_AllowFileCacheOfRasterImage ||
                IsOnlineMap && !sara.gisserver.console.config.ConfigManager.App_AllowFileCacheOfOnlineMaps)
                return;
