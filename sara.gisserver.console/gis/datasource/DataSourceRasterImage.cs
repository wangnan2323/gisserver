...
namespace sara.gisserver.console.gis.datasource
{
    public class DataSourceRasterImage:DataSourceBase
    {
        ...
        public DataSourceRasterImage(string path, string tilingSchemePath,string serviceName)
        {
            ...
            Initialize(path);
            if (sara.gisserver.console.config.ConfigManager.App_AllowFileCacheOfRasterImage)
            {
                //init local cache file if does not exist.
                string localCacheFileName = sara.gisserver.console.config.ConfigManager.App_FileCachePath + "\\" + serviceName.Trim().ToLower() + ".cache";
