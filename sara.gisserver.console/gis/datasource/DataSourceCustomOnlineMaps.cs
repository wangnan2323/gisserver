...
namespace sara.gisserver.console.gis.datasource
{
    public class DataSourceCustomOnlineMaps:DataSourceBase,IFormatConverter
    {
        ...
        protected override void Initialize(string path)
        {
            ...
            if (_mapName.ToLower().Contains("bing") && _mapName.ToLower().Contains("image") && !string.IsNullOrEmpty(sara.gisserver.console.config.ConfigManager.App_BingMapsAppKey))
                _baseUrl += "&token=" + sara.gisserver.console.config.ConfigManager.App_BingMapsAppKey;
        }
